...
public class ObjectEditionAgent implements DiagramAgent {
    ...
    public String editObjectProperties(@ToolParam(description = "Explain what properties to modify with their new values.") String orchestratorPrompt, @ToolParam(description = "The object id to edit, the id is in a format similar to \"AbcdEF+GhijKLM1NOpqrS==\".") String objectId) {
        this.setToolsInput();
        ...
        prompt.getInstructions().add(new org.springframework.ai.chat.messages.UserMessage("Here is the object to edit: " + objectId + ". " + orchestratorPrompt));

        return chatClient.prompt(prompt).tools(objectEditionTools).call().content();
    }
}
